...
The communications foundation includes conversations, direct/group messaging, participants, messages, attachments, read cursors, archive/mute behaviour, role-aware permissions, notification preferences, in-app notifications and announcements.

Email/SMS/WhatsApp delivery has adapter architecture, but provider activation depends on real secrets and verified edge-function configuration. Outbox-style delivery is separated from the in-app record of a notification.

### Security
Tenant/RBAC controls and RLS apply to persisted communication data. Attachments and recipient visibility must remain scoped to the relevant users/tenant.