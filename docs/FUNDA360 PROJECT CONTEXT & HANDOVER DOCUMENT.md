...
- The app now covers: authentication, multi-tenancy, school administration, user/role management, academic structure (including teaching assignments), employee management, learner management (including guardians, medical information, emergency contacts, documents, enrollment/promotion history), self-service views (including "my classes" for teachers), daily attendance, and reports/analytics — each tenant-scoped and RBAC-gated. Timetabling, gradebook, and finance remain future epics.
...
- **The Attendance Domain shipped next** — daily class registers (present/absent/late), class-scoped recording rights (a teacher may only mark attendance for a class they're actually assigned to; school_owner/principal may mark any class), wired into the dashboard's previously-honest-empty-state Attendance Overview panel with real counts. See §35.
...
## 35. Attendance Domain (2026-08-17)

**Scope selection:** requested directly by the product owner (not sourced from a PRD/ADR — none exists for attendance). Built from a concrete example: a pasted daily class roster (learner name → Present/Absent/Late, one class, one day, no period/subject dimension). The schema, permission model, and UI were all built to match exactly that shape — no timetable/period concept was invented, since none exists anywhere else in this codebase to attach one to.

**Database (`supabase/migrations/20260817120000_attendance.sql`):** a single new table, `attendance_records` — one row per `(learner_id, class_id, attendance_date)`, enforced by a unique index so re-saving a register corrects existing rows via `upsert` rather than duplicating them. `status` is a new Postgres enum, `attendance_status ('present' | 'absent' | 'late')` — deliberately only the three values the pasted example showed; no `excused`/`sick`/etc. invented. Never hard-deleted (no DELETE policy, same as every other table in this schema) — a mismarked entry is corrected via UPDATE.

**Recording rights are class-scoped, not a single blanket permission** — this is new: every other `.manage`-tier permission in this app (`academic.manage`, `employee.manage`, `learner.manage`) is a flat yes/no per school. Attendance needed something narrower, because in reality a teacher records attendance only for their own class, not the whole school. Solved with a new SQL function, `can_record_attendance(school_id, class_id)`: true for `school_owner`/`principal` (reuses `can_manage_academic()`, broad), OR true if the caller holds an *active* `class_teacher_assignments` row for that exact `class_id` (any `subject_id` — this schema has no homeroom-teacher-vs-subject-teacher distinction to invent one from, so any teacher assigned to the class may take its daily register). Viewing reuses `can_view_academic()` wholesale — the same "a teacher already sees the whole academic structure" reasoning `class_teacher_assignments` (§31) documented for the same role set.

**RBAC (`src/features/rbac/`):** two new permissions, `attendance.view`/`attendance.manage`, granted to exactly the same role set as `can_view_academic()`'s SQL role list (`school_owner`, `principal`, `teacher`, `class_teacher`, `subject_teacher`, plus platform admins). Both permissions currently carry an identical role list — the asymmetry is enforced at the RLS layer (SELECT is unconditional per that role list; INSERT/UPDATE is additionally scoped per-class via `can_record_attendance()`), not by the flat TS permission itself. The UI mirrors this: `attendance.manage` gates whether the Mark Attendance page is reachable at all; which specific classes a given signed-in user can actually pick from and save is scoped separately (all active classes for `school_owner`/`principal`, only `useMyTeachingAssignments()`'s classes for teacher-variant roles) — so the class list offered never contains a class the RLS layer would reject a save for.

**Feature module (`src/features/attendance/`)**, following the established `{types,services,hooks,pages}` shape: `attendanceService` (`getClassRoster` — enrolled learners for a class/year via `learner_enrollments` then `learners`, two flat queries rather than an embedded join, matching this codebase's existing style; `getAttendanceForClassDate`; `saveAttendance` — a single `.upsert(rows, {onConflict: 'learner_id,class_id,attendance_date'})` call, not a custom RPC, since no cross-row atomicity or elevated-privilege need justified one; `getSchoolAttendanceSummary` — status counts for one school/date, summed client-side over a small unpaginated result set), `useAttendanceRoster`/`useAttendanceSummary` hooks, `AttendancePage` (class + date pickers, a roster table with a three-way Present/Absent/Late button group per learner, a Save action). Wired into `AppRoutes.tsx` as `/attendance` under a new `RequirePermission permission="attendance.view"` gate, and into `DashboardSidebar.tsx` (the "Attendance" item, previously a permanent `Soon`-badged placeholder with no backing route at all since the frontend redesign in §34, is now a real link for any role holding `attendance.view`).

**Dashboard (`src/pages/DashboardPage.tsx`):** the "Attendance Overview" panel — an honest "not yet available in this workspace" empty state as of §34, written at a time when this domain genuinely didn't exist — now shows real Present/Absent/Late counts for the school, for today, sourced from `useAttendanceSummary`. If no register has been taken anywhere in the school yet today, it still shows an honest "No attendance recorded for today yet" state rather than a misleading all-zero chart. A new **System Status** row, "Attendance Records," reports Online/Degraded from that same hook's real error state, matching the pattern every other System Status row already used.

**A real bug was found and fixed during e2e verification, not just a flake:** after a successful save, `AttendancePage` originally called the roster hook's `refetch()` before setting the "Register saved." success flag — but the roster/`existingRecords`-sync `useEffect` unconditionally resets that same flag to `false` on every `existingRecords` change (needed so switching class/date clears a *stale* banner), and effects always run after a render regardless of statement order inside the async handler that triggered it — so the refetch-triggered re-render silently reset the just-set success flag before it ever painted. Confirmed with a standalone Playwright-core repro outside the test harness (proved the page and save request both work correctly) before concluding the failure was real, not environmental. Fixed by not refetching after save at all — `saveAttendance`'s call succeeding already confirms the local `statuses` state (what was actually submitted) is correct, so there is nothing to refetch, and the race disappears entirely rather than being reordered around.
...
| TypeScript | clean |
| ESLint | clean |
| Vitest | 45/45 (unchanged — no new unit-tested pure logic; the domain is CRUD + RLS) |
| Vite build | clean; `AttendancePage` is its own lazy chunk (~6.4 kB) |
| Playwright | 89/89 (84 existing + 5 new in `e2e/attendance.spec.ts`) across two clean runs, once the same pre-existing environmental noise documented in §34 was accounted for |
| Visual verification | performed via the same `playwright-core` technique as §33/§34 — both the Mark Attendance page (roster rendering, status buttons, save flow) and the Dashboard's populated Attendance Overview panel were rendered and reviewed directly, not inferred from source |
| RLS Docker regression suite (`supabase/rls-tests/`) | **not run** — Docker is not available in this environment. The RLS policies and `can_record_attendance()` follow the exact same structure as `class_teacher_assignments`' already-proven policies (§31), but this specific migration has not been executed against the Docker harness the way every prior migration in this schema was. Flagged as a real gap, not silently skipped. |

**Not changed:** no existing table, RLS policy, permission, or route was modified — this is a pure addition. No timetable/period/subject dimension was invented for attendance; it stays exactly as evidenced (one status per learner per class per day).

**Deferred, not done in this sprint:** guardian-facing attendance visibility (a parent seeing their own child's attendance history) was deliberately left out — it would need both a new RLS SELECT clause (an easy, low-risk addition following the `is_learner_guardian()` pattern already used for medical/emergency-contact visibility) and a new UI surface to actually consume it, and building the RLS half without the UI half would be a capability nobody could reach, which is the failure mode the "no fabricated/unused capability" discipline the rest of this project holds. An "excused" status was deliberately not added (no evidence called for it). A per-class attendance history/trend view beyond "today only" was not built — the dashboard summary is today-only by design of this pass.
