...
**Status:** Living document. Regenerated 2026-08-16 during a dedicated Production Hardening sprint that followed a full repository audit — Sprint 1, Sprint 2 (Milestones 1–4), and Sprint 3 (Milestones 1–3, self-service/guardian access) are all complete and committed to `main` as of this regeneration. This revision corrects several sections that had gone stale (this document had not been updated since immediately after Sprint 1, despite five further milestones and a full sprint landing since) — see §27–§29 for what changed and why, and §16.1 for a correction to a since-fixed finding this document previously left open.
...
- The app now covers: authentication, multi-tenancy, school administration, user/role management, academic structure, employee management, learner management (including guardians, medical information, emergency contacts, documents, enrollment/promotion history), self-service views, and reports/analytics — each tenant-scoped and RBAC-gated. Attendance, timetabling, gradebook, and finance remain future epics; no Sprint 4 brief has been received as of this regeneration (see §30).
- The codebase has **zero TODO/FIXME/XXX placeholders**, passing lint/typecheck/unit test suites, and a clean `npm run build` as of this regeneration (verified directly, not carried forward from a prior pass — see §28 for the exact commands run and their results).
...
- **The `teacher`/`class_teacher`/`subject_teacher` role distinction is not backed by any data relationship** — all three resolve to identical permissions, and no table links a staff member to a specific class/subject/year. This is deliberately deferred architectural work, not an oversight — see ADR-0001 (`docs/adr/ADR-0001-teaching-assignment-domain.md`) for the full, still-accurate analysis (re-verified during this regeneration).
...
No Sprint 4 brief has been given as of this regeneration. Legitimate candidates surfaced by the audit and this hardening sprint, for the product owner to prioritize (none scoped or started):

- **New product functionality** — attendance, timetabling, gradebook/assessment, or finance/HR-payroll are the next epics per §1's long-term shape; none has been scoped yet.
- **Teaching-assignment domain** (ADR-0001) — if any of attendance/gradebook/teacher-portal/timetabling is chosen next, this relationship will very likely become a real, immediate dependency (the ADR already analyzes all four).
- **E2E in CI** — low-risk now that §28.6 established the suite needs no live backend; just a `playwright install` CI step away.
- **`profiles.role` SQL/TS mirror drift risk** (§23) and the **`@supabase/supabase-js` version-bump generic-typing regression test** (§23) — both long-standing, still-accepted tradeoffs, not urgent.
- **Role-assignment UI coverage** — only 3 of 24 roles have a UI path to be assigned (§24); worth a product decision on whether more are needed before Sprint 4 introduces roles that would want one.
