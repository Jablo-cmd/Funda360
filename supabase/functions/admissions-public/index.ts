...
Deno.serve(async (req) => {
  ...
  try {
    if (action === 'config') {
      ...
      const { data: school } = await s.from('schools').select('id, name, status').eq('id', schoolId).maybeSingle();
    ...
    if (action === 'upload-url') {
      ...
      const { data: app } = await s.from('admission_applications').select('id, school_id, status').eq('resume_token', token).maybeSingle();
    ...
    console.error('admissions-public', action, err);
