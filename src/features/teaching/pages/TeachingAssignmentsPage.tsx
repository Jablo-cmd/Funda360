...
export function TeachingAssignmentsPage() {
  ...
  const academicYearsById = useMemo(
    () => Object.fromEntries(academicYears.map((year) => [year.id, year])),
    [academicYears],
  );
  ...
  return (
    <PageContainer>
          ...
          school && (
            <div className="w-full sm:w-auto sm:min-w-[9rem]">
              <Button
                ...
                disabled={classes.filter((c) => c.active).length === 0}
              >
                Add assignment
              </Button>
            </div>
      ...
      {canManage && school && classes.filter((c) => c.active).length === 0 && (
        <p className="text-sm text-content-tertiary">
          Add an active class before creating assignments.
        </p>
      )}
      ...
      {!school ? (
        <NoActiveSchoolNotice resource="teaching assignments" />
      ) : isLoading ? (
