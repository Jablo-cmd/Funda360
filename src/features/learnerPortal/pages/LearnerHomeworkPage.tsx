...
export function LearnerHomeworkPage() {
  const { record, isLoading: recordLoading } = useMyLearnerRecord();
  ...
  const load = useMemo(
    () => async () => {
      ...
      setError(null);
      try {
        setItems(await homeworkService.listAssignmentsForLearners([record.id]));
      } catch (err) {
  ...
  if (recordLoading || isLoading) return <PageContainer><LoadingBlock label="Loading homework…" /></PageContainer>;
  ...
  return (
    <PageContainer>
      <PageHeader title="Homework" description="Assignments set for you." />
      <ErrorAlert message={error} />

      {selected ? (
        <div className="flex flex-col gap-3">
          <button type="button" onClick={() => navigate('/learner/homework')} className="text-sm text-content-tertiary underline">
            ← All homework
          </button>
          <h2 className="text-xl font-bold text-content-primary">{selected.assignment.title}</h2>
          <p className="text-sm text-content-secondary">
            {selected.assignment.subjectName} · {formatDue(selected.assignment.dueAt)}
          </p>
        ...
        <ul className="flex flex-col gap-2">
          {items.map((item) => (
            <li key={item.submission.id}>
              <button
                type="button"
                onClick={() => navigate(`/learner/homework/${item.assignment.id}`)}
              ...
              >
                ...
                <span className="text-sm text-content-secondary">
                  {item.assignment.subjectName} · {formatDue(item.assignment.dueAt)}
                </span>
              </button>
            </li>
          ))}
